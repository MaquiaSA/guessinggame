    ...
    col.find({}).limit(1).sort({ $natural: -1 }).toArray(function (err, docs) {
      ...
      if (docs[0].question[index] == chosen) {
        ...
        if (index == 3) {
          ...
          let time_diff = (gameEnd.getTime() - gameStart.getTime()) / 1000;
          let duration_secs = Math.abs(time_diff).toFixed(2);
          let duration_text = Math.floor(duration_secs / 60) + " mins, " + duration_secs + " sec"
